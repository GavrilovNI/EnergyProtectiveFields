...
public class FieldControllerMenu extends AbstractContainerMenu
{
    ...
    {
        super(ModMenuTypes.FIELD_CONTROLLER_MENU.get(), pContainerId);
        ...
        this.blockEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(itemHandler -> {
            int i = 0;
            this.addSlot(new SlotItemHandler(itemHandler, i++, 80, 36));
            this.addSlot(new SlotItemHandler(itemHandler, i++, 95, 36));
        });
    }
    ...
    private void addPlayerInventory(Inventory playerInventory)
    {
        for(int y = 0; y < 3; y++)
            for(int x = 0; x < 9; x++)
                addSlot(new Slot(playerInventory, x + y * 9 + 9, 8 + x * 18, 86 + y * 18));
    }
